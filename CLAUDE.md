...
- **Search (M14):** Postgres is the source of truth; the search index is a projection. Triggers (`packages/db/migrations/0010_search_triggers.sql`) record every change that could alter a search document in `search_outbox`; the indexer (`apps/api/src/search/indexer.ts`, running in the API) claims rows by deleting them, rebuilds the affected per-user documents from SQL (`search/docs.ts`) and writes them to the `SearchIndex` in `AppDeps.search` (Postgres `pg_trgm` in `search_docs`; Elasticsearch when configured). The query language (`artist:` `rating:>=4` …) is `parseSearchQuery` in `packages/core`, parsed by both web and API. New searchable data: add it to `docs.ts`, make sure a trigger enqueues its changes, and cover it in `search/contract.ts`, the suite every engine must pass. Tests index with `ctx.indexSearch()`; `pnpm search:reindex` rebuilds from scratch.
