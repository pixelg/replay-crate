...
pnpm search:up          # optional Elasticsearch 9 + Kibana (compose profile `search`)
pnpm lint               # oxlint
...
- **UI:** mobile-first. shadcn/ui (Base UI flavor, `apps/web/components.json`) with our palette on shadcn's token names in `apps/web/src/styles.css`: `bg-background`, `text-muted-foreground`, `bg-card`, `bg-muted`, `bg-primary` (the brand orange), `bg-accent` (subtle hover surface, *not* the brand colour), `text-destructive`, `chart-1..5`. Never raw colours. Light/dark is `data-theme` on `<html>` (set before paint by `index.html`, then `src/lib/theme.ts`); dark tokens and Tailwind's `dark:` key off it, and Storybook has a Theme toolbar. Add shadcn components with `pnpm dlx shadcn@latest add <name>` from `apps/web` (review what it writes); our own Base UI wrappers live beside them in `src/components/ui`. Merge classes with `cn()` from the `cn` package; import from `@/…` or relatively.
...
- **Search (M14):** Postgres is the source of truth; the search index is a projection. Triggers (`packages/db/migrations/0010_search_triggers.sql`) record every change that could alter a search document in `search_outbox`; the indexer (`apps/api/src/search/indexer.ts`, running in the API) claims rows by deleting them, rebuilds the affected per-user documents from SQL (`search/docs.ts`) and writes them to the `SearchIndex` in `AppDeps.search` (`search/engine.ts`: Elasticsearch in `search/elastic.ts` when `ELASTICSEARCH_URL` is set, with read/write aliases and external versioning; otherwise Postgres `pg_trgm` in `search_docs`). The query language (`artist:` `rating:>=4` …) is `parseSearchQuery` in `packages/core`, parsed by both web and API. New searchable data: add it to `docs.ts`, make sure a trigger enqueues its changes, and cover it in `search/contract.ts`, the suite every engine must pass (CI's `search` job runs it on a real Elasticsearch; locally `pnpm search:up` then `ELASTICSEARCH_URL=http://127.0.0.1:9200 pnpm --filter @replay-crate/api exec vitest run src/search`). A mapping change makes a new index hash: run `pnpm search:reindex`. Kibana analytics (`search/analytics.ts`, Elasticsearch only): play events ride the indexer into `rc-plays`, settled searches come in through `POST /search/events` into `rc-search-queries`; the dashboards are code in `infra/kibana/objects.ts` (`pnpm kibana:load`). Tests index with `ctx.indexSearch()`; `pnpm search:reindex` rebuilds from scratch.
