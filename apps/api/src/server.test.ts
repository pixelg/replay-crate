...
describe('createServer with the built web app', () => {
  ...
  it('keeps the API as JSON, including unknown routes and errors', async () => {
    expect(await (await server.request('/api/health')).json()).toEqual({ ok: true })

    const missing = await server.request('/api/nope')
    ...
    const crashed = await server.request('/api/sync', {
      method: 'POST',
