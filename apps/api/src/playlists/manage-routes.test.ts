...
describe('playlist management', () => {
  ...
  const syncLibrary = () => send('POST', '/api/playlists/sync')
  ...
  describe('changing tracks', () => {
    ...
    it('adds tracks at a position and stores what Spotify now has', async () => {
      const res = await send('POST', '/api/playlists/road/items', { trackIds: ['x', 'y'], position: 1 })
    ...
    it('removes every copy of a track', async () => {
      await send('POST', '/api/playlists/road/items', { trackIds: ['a'] })
      const res = await send('DELETE', '/api/playlists/road/items', { trackIds: ['a'] })
      expect(res.status).toBe(200)
    ...
    it.each([
      ...
      const res = await send('PUT', '/api/playlists/road/items/move', { from, to })
      expect(res.status).toBe(200)
    ...
    it('refuses playlists outside the library', async () => {
      const res = await send('POST', '/api/playlists/not-mine/items', { trackIds: ['a'] })
    ...
    it('reports Spotify refusing the change as forbidden', async () => {
      ...
      const res = await send('POST', '/api/playlists/road/items', { trackIds: ['x'] })
      expect(res.status).toBe(403)
      expect(await json(res)).toEqual({ error: 'forbidden' })
    })

    it('validates the body', async () => {
      const res = await send('PUT', '/api/playlists/road/items/move', { from: -1, to: 'top' })
  ...
  describe('creating', () => {
    it('creates a playlist with tracks, in batches of 100, and adds it to the library', async () => {
      ...
      const res = await send('POST', '/api/playlists', { name: 'Big one', trackIds })
      ...
      const list = await json(await send('GET', '/api/playlists'))
  ...
  describe('rules', () => {
    ...
    const preview = async (rule: Record<string, unknown>) => json(await send('POST', '/api/playlists/preview', { rule }))
    ...
    it('rejects unknown rules', async () => {
      const res = await send('POST', '/api/playlists/preview', { rule: { kind: 'vibes' } })
      expect(res.status).toBe(400)
    })
  })
})
