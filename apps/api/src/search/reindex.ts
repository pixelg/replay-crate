...
import { createAnalytics, createSearchIndex, isElastic } from './engine.ts'
...
const analytics = createAnalytics({ elasticsearchUrl: ENV.ELASTICSEARCH_URL })
...
if (isElastic(search) && !userId) {
  ...
  await analytics?.ensureIndices()
  const next = await search.beginRebuild()
  console.log(`Building ${next}; searches use the current index until it's ready…`)
  const written = await buildEverything({ db, search, analytics }, { log: (message) => console.log(`  ${message}`) })
  ...
  const result = await drainAll({ db, search, analytics })
